...
// H(s) = w / (s + w)
class FilterF1 final : public FilterIIR {
public:
  ...
  Eigen::VectorXd filt(Eigen::VectorXd& x) override;

  Eigen::VectorXd filt(Eigen::VectorXd& x, double dt);

  inline void set(Eigen::VectorXd& x0) { x1 = x0; y1 = x0; }

  void update(double cutOff, double sampTime) override;
...
// H(s) = -w^2 / (s + w)
class FilterF2  final: public FilterIIR {
public:
  ...
  inline void set(Eigen::VectorXd& x0) { x1 = x0; y1 = -f2*omega*x0; }
...
class FilterButterworth final : public FilterIIR {
public:
...
class FilterLowPass final : public FilterIIR {
public:
...
class FilterHighPass final : public FilterIIR {
public:
...
void FilterButterworth::update(double cutOff, double sampTime)
{
  ...
  double denom = 1 + M_SQRT2*omega + omega2;

  ky1 = 2*(1-omega2)/denom;
  ky2 = (M_SQRT2*omega-1-omega2)/denom;
